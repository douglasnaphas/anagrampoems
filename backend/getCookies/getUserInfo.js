 ...
 *   post: res.locals contains properties email, sub, and
...
const getUserInfo = (jwt) => {
  const middleware = (req, res, next) => {
    try {
      ...
      logger.log(
        "getUserInfo: unable to get email/sub/" + "cognito:username"
      );
      logger.log(err);
    ...
    if (
      ...
      logger.log(
        "getUserInfo: invalid email, sub, or " + "cognito:username:"
      );
